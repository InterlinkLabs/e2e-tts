...
from .constants import PAD, TOKEN_SELF_ATTN_VALUE
from .utils import *
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        d_head = hidden_dim // self.config["decoder_head"]
        d_model = hidden_dim
...
### Code building copy and modify from: https://github.com/lucidrains/https://github.com/lucidrains/reformer-pytorch ###
...
class LSHAttention(nn.Module):
    ...
    def hash_vectors(self, n_buckets, vecs):
        ...
        rotated_vecs = torch.torch.einsum('btf,bfhi->bhti', dropped_vecs, random_rotations)
    ...
    def forward(self, qk, v, query_len = None, input_mask = None, input_attn_mask = None, pos_emb = None, **kwargs):
        ...
        dots = torch.torch.einsum('bhie,bhje->bhij', bq, bk) * (dim ** -0.5)
        ...
        bo = torch.torch.einsum('buij,buje->buie', dropped_dots, bv)
...
class FullQKAttention(nn.Module):
    ...
    def forward(self, qk, v, query_len = None, input_mask = None, input_attn_mask = None, **kwargs):
        ...
        dot = torch.torch.einsum('bie,bje->bij', q, qk) * (dim ** -0.5)
        ...
        out = torch.torch.einsum('bij,bje->bie', dot, v)
...
class LSHSelfAttention(nn.Module):
    ...
    def forward(self, x, keys = None, input_mask = None, input_attn_mask = None, context_mask = None, pos_emb = None, **kwargs):
        ...
        merge_batch_and_heads = functools.partial(merge_dims, 0, 1)
        ...
        split_index_fn = functools.partial(split_at_index, 1, l_h)
        ...
        functools.partial_attn_fn = functools.partial(attn_fn, query_len = t, pos_emb = pos_emb, **kwargs)
        attn_fn_in_chunks = process_inputs_chunk(functools.partial_attn_fn, chunks = self.attn_chunks)
...
class LocalAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask = None, input_mask = None):
        ...
        # https://github.com/arogozhnikov/einops/blob/master/docs/4-pack-and-einops.unpack.ipynb
        (q, packed_shape), (k, _), (v, _) = map(lambda t: einops.pack([t], '* n d'), (q, k, v))
        ...
        b_t = einops.rearrange(seq, '(w n) -> 1 w n', w = windows, n = window_size)

        bq, bk, bv = map(lambda t: einops.rearrange(t, 'b (w n) d -> b w n d', w = windows), (q, k, v))
        ...
        sim = torch.einsum('b h i e, b h j e -> b h i j', bq, bk) * scale
        ...
        if exists(mask):
            ...
            mask = einops.rearrange(mask, '... (w n) -> (...) w n', w = windows, n = window_size)
            mask = look_around(mask, **{**look_around_kwargs, 'pad_value': False})
            mask = einops.rearrange(mask, '... j -> ... 1 j')
            mask = einops.repeat(mask, 'b ... -> (b h) ...', h = h)
...
class Reformer(nn.Module):
    def __init__(self, dim, depth, heads = 8, dim_head = None, bucket_size = 64, n_hashes = 8, ff_chunks = 100, attn_chunks = None, causal = False, weight_tie = False, lsh_dropout = 0., ff_dropout = 0., ff_activation = None, ff_mult = 4, ff_glu = False, post_attn_dropout = 0., layer_dropout = 0., lsh_attend_across_buckets = True, lsh_allow_duplicate_attention = True, random_rotations_per_head = False, use_scale_norm = False, use_rezero = False, use_full_attn = False, full_attn_thres = 0, reverse_thres = 0, num_mem_kv = 0, one_value_head = False, n_local_attn_heads = 0, pkm_layers = tuple(), pkm_num_keys = 128):
        ...
        residual_fn_wrapper = ReZero if use_rezero else functools.partial(PreNorm, norm_type, dim)
